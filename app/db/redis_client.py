...
class ZuRedisClient(object):
    ...
    def open_redis_client(cls):
        ...
        if cls.redis_client is None:
            ...
            if RedisConfig.REDIS_USE_SENTINEL:
                sentinel = aioredis.sentinel.Sentinel(
                    [(RedisConfig.REDIS_HOST, RedisConfig.REDIS_PORT)],
                ...
                if RedisConfig.REDIS_PASSWORD:
                    cls.redis_client = aioredis.from_url(
                        f"redis://{RedisConfig.REDIS_USER}:{RedisConfig.REDIS_PASSWORD}"
                        f"@{RedisConfig.REDIS_HOST}:{RedisConfig.REDIS_PORT}",
    ...
    @classmethod
    async def set_redis_client_name(cls):
        """Set Redis client name."""
        if cls.redis_client:
            await cls.redis_client.client_setname(RedisConfig.CLIENT_NAME)
    ...
    @classmethod
    async def set(cls, key: str, value: str, ext: Optional[int] = None) -> bool:
        ...
        Args:
            ...
            ext (int, optional): Expiration time in seconds.

        Returns:
            bool: Return true if set successfully else False.
        ...
        try:
            res = await redis_client.set(name=key, value=value, ex=ext)
    ...
    async def exists(cls, key):
        ...
        cls.log.debug("Preform Redis EXISTS command, key: {}, exists".format(key))
        try:
    ...
    async def get(cls, key):
        ...
        Get the value of key. If the key does not exist the special value False
    ...
    @classmethod
    async def hmget(cls, key, fields) -> list:
        """Execute Redis HGET command.
        Returns the values associated with the specified fields in the hash stored at
        key.. If the key does not exist or field does not exist in key the special
        value None is returned. An error is returned if the value stored at key is not a
        string, because GET only handles string values.

        Args:
            key (str): Redis db key.
            fields (list): Fields to fetch values for in the give Redis db key hash.

        Returns:
            response (list): Array of values for the given fields, in the same order as
            they are requested.
        ...
        cls.log.debug("Preform Redis HGET command, key: {}".format(key))
        try:
            return await redis_client.hmget(name=key, keys=fields)
        except RedisError as ex:
            cls.log.exception(
                "Redis HGET command finished with exception",
                exc_info=(type(ex), ex, ex.__traceback__),
            )
            raise ex
